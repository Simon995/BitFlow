...
// 声明在 mec_conv.cu 中定义的 CUDA 函数
// extern "C" 确保 C++ 编译器使用 C 链接约定，这样可以找到函数定义
// custom_conv2d_gpu
// custom_conv2d_gpu_cutlass_approach
extern "C" void custom_conv2d_gpu(
...
// PyTorch 包装函数
torch::Tensor mec_conv_forward(
        torch::Tensor input,      // (batch_size, in_channels, in_height, in_width)
        torch::Tensor weight,     // (out_channels, in_channels, kernel_h, kernel_w)
    ...
    // weight 张量的原始形状是 (out_channels, in_channels, kernel_h, kernel_w)
    // kernel_h 和 kernel_w 可以从 weight.size(2) 和 weight.size(3) 获取
    ...
    TORCH_CHECK(out_height > 0 && out_width > 0, "Output height and width must be positive. Check padding, stride, and kernel size.");


    // 准备权重张量：将其扁平化为 (out_channels, in_channels * kernel_h * kernel_w)
    // .contiguous() 确保内存是连续的，然后 .view() 进行变形
    torch::Tensor weight_flat = weight.contiguous().view({out_channels, in_channels * kernel_h * kernel_w});
    CHECK_CONTIGUOUS(weight_flat); // 确保变形后的张量仍然是连续的 (view 通常保持连续性如果原张量是)
...
&mec_conv_forward, // C++中对应的函数指针
"MEC Custom Conv2D forward (CUDA)" // 函数的文档字符串
