...
__global__ void fused_im2col_gemm_bias_kernel(
        const float* d_kernel_flat, // 权重 (out_channels, in_channels * kernel_h * kernel_w)
        const float* d_src,         // 输入 (in_channels, in_height, in_width) for one batch
        const float* d_bias,        // 偏置 (out_channels)，如果不需要偏置，可以传入 nullptr
        float* d_output,            // 输出 (out_channels, out_height, out_width) for one batch
    ...
    // 共享内存声明
    // 编译器通常会优化对齐，但显式使用 alignas 可以更明确 (C++11起)
    // alignas(16) // 例如，确保16字节对齐
    ...
    for (int k_block_outer = 0; k_block_outer < (K_orig + TILE_DIM - 1) / TILE_DIM; ++k_block_outer) {
        ...
        if (m_global < M_orig && k_a_global < K_orig) {
            ...
            // 修正：A_tile 的行索引应该是 threadIdx.y (m_a_local)
            A_tile[m_a_local][k_a_local] = d_kernel_flat[(size_t)m_global * K_orig + k_a_global];
        } else {
        ...
        if (n_global < N_orig && k_b_global < K_orig) {
            ...
            if (ih_idx >= 0 && ih_idx < inHeight_param &&
                ...
                // B_tile[k_b_local][n_b_local] = d_src[(size_t)channel_idx * inHeight_param * inWidth_param + (size_t)ih_idx * inWidth_param + iw_idx];
                // 修正：B_tile 的列索引应该是 threadIdx.x (n_b_local)
        ...
        if (m_global < M_orig && n_global < N_orig) {
            // 考虑循环展开以优化此内部循环 (如果 TILE_DIM 是固定的较小值)
            // #pragma unroll
